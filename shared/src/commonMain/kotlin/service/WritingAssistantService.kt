...
class WritingAssistantService {
    ...
    /**
     * Checks if text ends with a sentence-ending punctuation.
     * Triggers analysis when a new sentence is detected.
     */
    fun checkAndAnalyze(text: String, cursorPosition: Int): Boolean {
        if (text.isBlank() || analysisCooldown) return false
        
        // Find the current sentence being written
        val beforeCursor = text.substring(0, cursorPosition.coerceAtMost(text.length))
        
        // Check if user just typed a sentence-ending punctuation
    ...
    /**
     * Force analysis of the current text.
     */
    fun analyzeCurrentText(text: String) {
        ...
        // Analyze the last complete sentence or the whole text if no sentence
        val sentences = text.split(Regex("(?<=[.!?])\\s+"))
        val lastCompleteSentence = sentences.lastOrNull { it.trim().isNotEmpty() }
            ?: text.trim()
    ...
    private fun extractLastSentence(text: String): String {
        ...
        // Find the previous sentence end (if any)
    ...
    /**
     * Analyze a sentence and generate comprehensive suggestions.
     */
    private fun analyzeSentence(sentence: String) {
        _isAnalyzing.value = true
        lastAnalyzedText = sentence
        
        // Set cooldown to prevent too frequent analyses
        analysisCooldown = true
        
        // Simulate analysis (in real implementation, this would call Ollama or local NLP)
        val analysis = performLocalAnalysis(sentence)
        
        _currentAnalysis.value = analysis
        _quickSuggestions.value = generateQuickSuggestions(analysis)
     ...
     */
    private fun performLocalAnalysis(sentence: String): WritingAnalysis {
    ...
    /**
     * Generate quick action suggestions from analysis.
     */
    private fun generateQuickSuggestions(analysis: WritingAnalysis): List<QuickSuggestion> {
